...
export function TaxRatesPageClient({ initialRates }: TaxRatesPageClientProps) {
  ...
  const provinces = useMemo(() => {
    ...
    const unique = Array.from(
      new Set(filtered.map((r) => r.stateOrProvinceCode))
    );
    return unique.sort();
  ...
  const filteredRates = useMemo(() => {
    ...
    if (selectedProvince !== "all") {
      filtered = filtered.filter(
        (r) => r.stateOrProvinceCode === selectedProvince
      );
    }
  ...
  // Filter federal brackets based on selected country
  const filteredFederalBrackets = useMemo(() => {
    if (selectedCountry === "all") {
      // Show only current year for each country
      const currentYear = new Date().getFullYear();
      const countryGroups = federalBrackets.reduce((acc, bracket) => {
        if (!acc[bracket.countryCode]) {
          acc[bracket.countryCode] = [];
        }
      ...
      // Filter by selected country and show only current year
      const countryBrackets = federalBrackets.filter(
        (b) => b.countryCode === selectedCountry
      );
  ...
  useEffect(() => {
    if (selectedCountry === "all") {
      ...
      // Keep province filter if it's still valid for the selected country
      const validProvinces = rates
        .filter((r) => r.countryCode === selectedCountry)
        .map((r) => r.stateOrProvinceCode);
      if (!validProvinces.includes(selectedProvince)) {
        setSelectedProvince("all");
      }
    }
    ...
    <div className="w-full p-4 lg:p-8">
      <div className="space-y-2 mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          These rates are used to calculate after-tax income for budgets and financial planning.
        </p>
      </div>
      ...
      <div className="mb-6">
              ...
              <SelectContent>
                ...
                {provinces.map((province) => {
                  const rate = rates.find(
                    (r) =>
                      r.stateOrProvinceCode === province &&
        ...
        {hasActiveFilters && (
          <div className="mt-4 text-sm text-muted-foreground">
            {selectedCountry !== "all" && (
              <span>
                Federal: {filteredFederalBrackets.length} brackets |{" "}
              </span>
            )}
      ...
      {/* Simple Tabs for Federal and Provincial */}
      <SimpleTabs defaultValue="federal" className="w-full">
        ...
        <SimpleTabsContent value="federal" className="mt-6">
          <div className="space-y-2 mb-4">
            ...
            <p className="text-sm text-muted-foreground">
              Progressive tax brackets for federal income tax calculations
            </p>
          </div>
          {loadingBrackets ? (
            ...
            <FederalBracketsTable
              brackets={filteredFederalBrackets}
              loading={false}
        ...
        <SimpleTabsContent value="provincial" className="mt-6">
          <div className="space-y-2 mb-4">
            ...
            <p className="text-sm text-muted-foreground">
              Flat or effective tax rates for state/provincial income tax calculations
            </p>
          </div>
          {loading ? (
            <div className="flex items-center justify-center py-8">
