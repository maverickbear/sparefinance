...
export default function CategoriesPage() {
  ...
  async function handleDeleteCategory(id: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorMessage.includes("Cannot delete system default")) {
          throw new Error("You cannot delete system default categories. Only your custom categories can be deleted.");
        }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorMessage.includes("Cannot delete")) {
          throw new Error("You cannot delete subcategories from system default categories.");
        }
        if (errorMessage.includes("Unauthorized")) {
          throw new Error("You are not authorized to delete this subcategory.");
        }
        throw new Error(errorMessage);
      }
  ...
  async function handleDeleteGroup(macroId: string) {
    ...
    try {
      const response = await fetch(`/api/macros/${macroId}`, { method: "DELETE" });
      const data = await response.json();
      
      if (!response.ok) {
        const errorMessage = data.error || "Failed to delete group";
        if (errorMessage.includes("Cannot delete system default")) {
          throw new Error("You cannot delete system default groups. Only your custom groups can be deleted.");
        }
        if (errorMessage.includes("Unauthorized")) {
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {category.subcategories.map((subcat) => (
                                        ...
                                        >
                                          ...
                                          {subcat.userId !== null && subcat.userId !== undefined && (currentUserId ? subcat.userId === currentUserId : false) && (
                                            <Button
                                              variant="ghost"
                                              size="icon"
