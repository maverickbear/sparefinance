 ...
 * Detects subscriptions from transaction history
 * 
 * SIMPLIFIED: Integrated into TransactionsService instead of separate service
 * This helper contains the detection logic and can be used by TransactionsService
 */
 ...
 * SIMPLIFIED: Integrated into TransactionsService, uses repository instead of direct DB access
 */
export async function detectSubscriptionsFromTransactions(
  ...
  try {
    logger.info(`[SubscriptionDetection] Detecting subscriptions for user: ${userId}`);
    ...
    // Use repository to get transactions (expense only, last 6 months)
    ...
    if (!transactions || transactions.length === 0) {
      logger.debug("[SubscriptionDetection] No transactions found");
      return [];
    }
    
    // Get accounts for transaction mapping
    const accountIds = [...new Set(transactions.map(tx => tx.account_id))];
    const accounts = await accountsRepository.findByIds(accountIds);
    ...
    logger.info(`[SubscriptionDetection] Detected ${detectedSubscriptions.length} potential subscriptions`);
    
    return detectedSubscriptions;
  } catch (error) {
    logger.error("[SubscriptionDetection] Error detecting subscriptions:", error);
