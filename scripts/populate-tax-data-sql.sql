...
VALUES
  ('US', 2024, 1, 0, 11600, 0.10, true),
  ('US', 2024, 2, 11600, 47150, 0.12, true),
  ('US', 2024, 3, 47150, 100525, 0.22, true),
  ('US', 2024, 4, 100525, 191950, 0.24, true),
  ('US', 2024, 5, 191950, 243725, 0.32, true),
  ('US', 2024, 6, 243725, 609350, 0.35, true),
  ('US', 2024, 7, 609350, NULL, 0.37, true)
ON CONFLICT ("countryCode", "taxYear", "bracketOrder") DO NOTHING;

-- Canada Federal Tax Brackets 2024
INSERT INTO system_tax_federal_brackets ("countryCode", "taxYear", "bracketOrder", "minIncome", "maxIncome", "taxRate", "isActive")
VALUES
  ('CA', 2024, 1, 0, 55867, 0.15, true),
  ('CA', 2024, 2, 55867, 111733, 0.205, true),
  ('CA', 2024, 3, 111733, 173205, 0.26, true),
  ('CA', 2024, 4, 173205, 246752, 0.29, true),
  ('CA', 2024, 5, 246752, NULL, 0.33, true)
ON CONFLICT ("countryCode", "taxYear", "bracketOrder") DO NOTHING;

-- Canada Federal Tax Brackets 2025
INSERT INTO system_tax_federal_brackets ("countryCode", "taxYear", "bracketOrder", "minIncome", "maxIncome", "taxRate", "isActive")
VALUES
  ('CA', 2025, 1, 0, 57375, 0.145, true),
  ('CA', 2025, 2, 57375, 114750, 0.205, true),
  ('CA', 2025, 3, 114750, 177882, 0.26, true),
  ('CA', 2025, 4, 177882, 253414, 0.29, true),
  ('CA', 2025, 5, 253414, NULL, 0.33, true)
ON CONFLICT ("countryCode", "taxYear", "bracketOrder") DO NOTHING;
